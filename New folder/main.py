...
def train_perceptron():
    ...
    print('S-layer generated ({} neurons)'.format(input_count))

    print('\n\nGenerating A-layer')
    #a_neurons_count = 2 ** input_count - 1 # на 3й запуск пишем тут = 300
    a_neurons_count = 5000
    ...
    print('A-layer generated ({} neurons)'.format(a_neurons_count))
    
    print('\n\nGenerating R-layer')
    for _ in range(NUMBER_COUNT):
        network.r_layer.add_neuron(a_neurons_count, lambda: 0, lambda value: 1 if value >=0 else -1, 0.01, 0)
    print('R-layer generated ({} neurons)'.format(NUMBER_COUNT))
